...
public static int CheckInputGroup(String oneGroupInfo){
		...
		int result=HttpMethod.httpGet2(CDCADMIN, CDCADMIN_PWD, CORP_JIRA_QA+Jira_GROUP+oneGroupInfo);
...
public static int CheckInputUser(String oneUserInfo){
		...
		int result=HttpMethod.httpGet2(CDCADMIN, CDCADMIN_PWD, CORP_JIRA_QA+Jira_USER+oneUserInfo);
...
public static String addJiraGroup(String validUser,ArrayList<String> groupList) throws UnsupportedEncodingException {
	    ...
	    //HashSet<String> Jira_Users = new HashSet<String>();
				  ...
				  for(String group :groupList){
					  ...
					  System.out.println("group:"+group);
				  int a = HttpMethod.httpPost(CDCADMIN,CDCADMIN_PWD,CORP_JIRA_QA + GROUP_USER + group, sb.toString());
		...
		HashSet<String> Jira_Users = new HashSet<String>();
			//generate post Json body
		       ...
		       /* String name="abidemib";
				...
				int a = HttpMethod.httpPost(CDCADMIN, CDCADMIN_PWD, CORP_JIRA_QA+Api,sb.toString());
					...
					for (int m = 0 ; m < groups.length ; m++) {
						  String group = groups[m];
							 for(String user : Jira_Users){
								 ...
								 int b = HttpMethod.httpDelete(CDCADMIN, CDCADMIN_PWD, CORP_JIRA_QA + GROUP_USER + group + DELETE_USER + curUser); 
								 if (b == 0) {
