...
public class LdapCheck {
	...
	public static JiraUser GetADInfo(String type,String searchInfo) {
		...
		/*
		 * String userName = name; // 用户名称 if (userName == null) { userName =
		 * ""; }
		 */
		String company = "";
//		String result = "";
		try {
			...
			{
				if(searchInfo.trim().length()==9 && searchInfo.indexOf(" ") == -1 && searchInfo.indexOf("-") ==-1 && searchInfo.charAt(3) >= '0' && searchInfo.charAt(3) <= '9')
				{
			...
			while (answer.hasMoreElements()) {// 遍历结果集
				...
				if (Attrs != null) {
					...
					try {
						...
						String fullName = map.get("cn");
						String mail = map.get("mail");
						
						String test = map.get("alcatelCIL");*/
						 
						//System.out.println("csl = " + uid);
						//System.out.println("fullName = " + fullName);
						//System.out.println("mail = " + mail);
						//System.out.println("alcatelCIL = " + test);
						/*String originalName=map.get("cn");
						String firstName=originalName.split(" ")[0];
						String lastName= originalName.substring(firstName.length()+1);
			...
			/*System.out.println("字符串长度："+(searchInfo.length()-1));
			System.out.println("************************************************");
	...
	public static void main(String args[]) {
		// 实例化
		LdapCheck ad = new LdapCheck();
		ad.initLdap();
		//System.out.println(ad.GetADInfo("inetorgperson","JIGAO"));
		JiraUser jiraUser=ad.GetADInfo("inetorgperson","ZZ0127780");
		if(jiraUser != null){
			System.out.println(jiraUser.toString());
		}else{
			System.out.println("Ldap中没有这个人！");
		}
		
		//test user:JiraUser [csl=abidemib, fullName=Abidemi BELLO, email=abidemi.bello@nokia.com]
		//chaume3
		// 查找用户
		/*
