...
public class DemoController {
	...
	public String loginToIndexPage(HttpServletRequest request) {
		logger.info("/login....");
		request.getSession().setAttribute("tishimessage", "请输入用户名和密码登录");
		request.getSession().removeAttribute("rgmessage");
	...
	public String registerToLoginPage(HttpServletRequest request) {
	...
	@ResponseBody
	public String userTest(String username, HttpServletResponse response) {
	...
	public String userRegister(String username, String password, HttpServletRequest request) {
		...
		request.getSession().setAttribute("rgmessage", "1");
		return "login";
	}

	@RequestMapping("/loginToHome")
	public  String loginChecked(String account, String password, Model model, HttpServletRequest request) {
		String back = "";
		if(request.getSession().getAttribute("username")==null){
			...
			if(!username1.equals("没人")&&!password1.equals("没密码")){
...
//				request.getSession().removeAttribute("password");
				...
				User u1 = userService.findUserByName(user);
				if (u1 != null) {
					System.out.println("密码错误");
					request.getSession().setAttribute("tishimessage", "密码错误");
				} else {
					System.out.println("账户不存在");
					request.getSession().setAttribute("tishimessage", "账户不存在");
				}
				back = "login";
			}
		}
	...
	public String redirectToIndexPage(HttpServletRequest request) {
		logger.info("index..");
		return "index";
	}

	@RequestMapping("/indextop")
	public String redirectToIndex(HttpServletRequest request,Model model) {
		logger.info("indextop..");
		return "indextop";
	}
}
